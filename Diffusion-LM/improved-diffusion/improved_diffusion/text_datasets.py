...
def load_data_highlevel(
    *, data_dir, batch_size, image_size, class_cond=False, deterministic=False, data_args=None, 
    ...
    highlevel_train = data_args.highlevel_train
    print(f'hello loading text data from {highlevel_train}')
    data = torch.load(highlevel_train)
    examples = data[split]
    ...
    #                                        padding_mode=padding_mode, split=split,
    #                                        load_vocab=load_vocab)
#dict_keys(['input_ids', 'labels', 'section_ids', 'seq_cl_feats', 'raw_text', 'sentence_embeddings', 'num_sentences', 'padded_sentence_embeddings'])
    def collate_fn(examples):
        examples = [{'padded_sentence_embeddings': ex['padded_sentence_embeddings']} for ex in examples]
...
def load_data_highlevel_subset(
    *, data_dir, batch_size, image_size, class_cond=False, deterministic=False, data_args=None, 
        task_mode='roc', model=None, padding_mode='block', split='train', load_vocab=None,
):
    """
