...
def get_logits(sequence, model, tokenizer, previous_features, current_features, device='cuda'):
    bos_token = "<|endoftext|>"
    eos_token = "<|endoftext|>"
    unk_token = "[UNK]"
    ...
    logits_value = model.forward(input_ids, cl_feats=current_features, past_seq_cl_feats=previous_total_features)["logits"][0][-1]

    logits = {}
    for i in range(len(logits_value)):
        for key, value in tokenizer.vocab.items():
            if value==i:
                logits[key]=logits_value[i].item()
