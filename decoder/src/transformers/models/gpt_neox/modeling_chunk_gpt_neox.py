...
class GPTNeoXPreTrainedModel(PreTrainedModel):
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, GPTNeoXChunkModel):
...
class GPTNeoXChunkModel(GPTNeoXPreTrainedModel):
    def __init__(self, config):
    ...
    def forward(
        ...
        if seq_cl_feats is not None:
            if self.dropout_diffusion is None:
                ...
                hidden_states = hidden_states + self.dropout_diffusion(seq_cl_feats)
...
class GPTNeoXChunkForCausalLM(GPTNeoXPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.gpt_neox = GPTNeoXChunkModel(config)
    ...
    def forward(
        ...
        if chunk_ids is not None:
            ...
            if self.config.encoder_mode.startswith('gaussian_'):
                gaussian = float(self.config.encoder_mode.split('_')[-1])
            else:
                gaussian = 0
            train_cl_feats = train_cl_feats + gaussian * torch.randn(*train_cl_feats.shape).to(train_cl_feats.device)

            if self.not_use_latent == 1:
                train_cl_feats = train_cl_feats.fill_(0)
        ...
        if cl_feats is not None:
            ...
            seq_cl_feats = cl_feats.clone().detach().unsqueeze(0).unsqueeze(1)
            if self.past_seq_cl_feats is None:
                self.past_seq_cl_feats = past_seq_cl_feats.unsqueeze(0)
            seq_cl_feats = torch.cat((self.past_seq_cl_feats, seq_cl_feats), dim=1)
            self.past_seq_cl_feats = seq_cl_feats

            # if past_seq_cl_feats is not None:
            #     past_seq_cl_feats = past_seq_cl_feats.unsqueeze(0)
            #     #import pdb; pdb.set_trace()
