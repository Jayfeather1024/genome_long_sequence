...
@dataclass
class DataCollatorForTimeControlChunk:
    tokenizer: PreTrainedTokenizerBase
    # padding: Union[bool, str, PaddingStrategy] = True
    # max_length: Optional[int] = None
    # pad_to_multiple_of: Optional[int] = None
    ...
    def __call__(self, features: List[InputDataClass]) -> Dict[str, torch.Tensor]:
        ...
        input_ids, attention_mask, chunk_ids, chunk_input_ids, chunk_attn_mask, labels = zip(*features)

        batch = {"input_ids": self._tensorize_batch(input_ids),
                 "labels": self._tensorize_batch(labels),
                 "attention_mask": self._tensorize_batch(attention_mask),
                 "chunk_ids": self._tensorize_batch(chunk_ids),
                 "chunk_input_ids": self._tensorize_batch(chunk_input_ids),
                 "chunk_attn_mask": self._tensorize_batch(chunk_attn_mask),}
...
@dataclass
class DataCollatorForTimeControlSame:
    tokenizer: PreTrainedTokenizerBase
    # padding: Union[bool, str, PaddingStrategy] = True
    # max_length: Optional[int] = None
    # pad_to_multiple_of: Optional[int] = None
    ...
    def __call__(self, features: List[InputDataClass]) -> Dict[str, torch.Tensor]:
        ...
        """
        #import pdb; pdb.set_trace()
        input_ids, labels, section_ids, cl_embeddings, raw_text, diffusion_mask = zip(*features)
        input_ids_expand = []
        labels_expand = []
        section_ids_expand = []
        cl_embeddings_expand = []
        raw_text_expand = []
        num_sents = diffusion_mask[0].shape[0]
        for sent_id in range(num_sents):
            curr_mask = diffusion_mask[0][sent_id]
            input_ids_expand.append(input_ids[0])
            labels_new = labels[0].data.clone()
            labels_new[1:][curr_mask.ne(1)[:-1]] = -100
            labels_expand.append(labels_new)
            section_ids_expand.append(section_ids[0])
            cl_embeddings_new = cl_embeddings[0].data.clone()
            cl_embeddings_new = cl_embeddings_new.gather(1, curr_mask.view(-1, 1, 1).expand(-1, 1, cl_embeddings_new.shape[2])).squeeze(1)
            cl_embeddings_expand.append(cl_embeddings_new)
            raw_text_expand.append(raw_text[0])
        #import pdb; pdb.set_trace()


        batch = {"input_ids": self._tensorize_batch(input_ids_expand),
                 "labels": self._tensorize_batch(labels_expand),
                 "section_ids": self._tensorize_batch(section_ids_expand),
                 "seq_cl_feats": self._tensorize_batch(cl_embeddings_expand),
                 "raw_text": raw_text_expand}
