...
def get_data_paths(data_args: DataTrainingArguments):
    ...
    assert data_args.dataset_name in [
        'wikisection', 'roc_stories', 'wikihow', 'recipe', 'tm2', 'tickettalk', 'tm2fixed', 'tm2sep', 'tm2new', 'tm2newsep', 'roc_stories_debug', 'roc_stories_train_with_diffusion', 'genome_sars']
    # Default datapaths
    ...
    if "genome_sars" in data_args.dataset_name:
        train_path = os.path.join(constants.PATH2GENOMESARS, "train.pkl")
        val_path = os.path.join(constants.PATH2GENOMESARS, "valid.pkl")
        test_path = os.path.join(constants.PATH2GENOMESARS, "test.pkl")
...
def get_dataset(
    ...
    if 'roc_stories_train_with_diffusion' == args.dataset_name:
    ...
    elif 'genome_sars' in args.dataset_name:
        dataset = StoriesDatasetChunk(
            tokenizer=tokenizer,
            file_path=file_path,
            block_size=args.block_size,
...
def main():
    ...
    a = torch.load('/home/jayfeather/language_modeling_via_stochastic_processes/genslm_model/model-epoch69-val_loss0.01.pt')['state_dict']
    b = {}
    for k in a:
