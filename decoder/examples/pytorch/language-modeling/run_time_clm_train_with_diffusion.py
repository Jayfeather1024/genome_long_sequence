...
from transformers import (
    ...
    DataCollatorForTimeControlSame,
    WikisectionDataset,
    StoriesDataset, StoriesTrainWithDiffusionDataset, StoriesTrainWithDiffusionSameDataset,
    RecipeDataset,
    TaskmasterDataset,
...
def get_data_paths(data_args: DataTrainingArguments):
    ...
    assert data_args.dataset_name in [
        'wikisection', 'roc_stories', 'wikihow', 'recipe', 'tm2', 'tickettalk', 'tm2fixed', 'tm2sep', 'tm2new', 'tm2newsep', 'roc_stories_debug', 'roc_stories_train_with_diffusion', 'roc_stories_train_with_diffusion_same']
    # Default datapaths
    ...
    if "roc_stories_train_with_diffusion_same" == data_args.dataset_name:
        DIFFUSION_GEN_PATH = data_args.diffusion_gen_path
        train_path = (os.path.join(constants.PATH2ROCSTORIES, "train.pkl"), os.path.join(DIFFUSION_GEN_PATH, 'zs.pt.masked.train.seeds.1234'))
        #train_path = (os.path.join(constants.PATH2ROCSTORIES, "valid.pkl"), os.path.join(DIFFUSION_GEN_PATH, 'zs.pt.masked.val.seeds.1234'))
        val_path = (os.path.join(constants.PATH2ROCSTORIES, "valid.pkl"), os.path.join(DIFFUSION_GEN_PATH, 'zs.pt.masked.val.seeds.1234'))
        test_path = (os.path.join(constants.PATH2ROCSTORIES, "test.pkl"), os.path.join(DIFFUSION_GEN_PATH, 'zs.pt.masked.test.seeds.1234'))
...
def get_dataset(
    ...
    elif 'roc_stories_train_with_diffusion_same' == args.dataset_name:
        assert args.diffusion_weight == 0
        dataset = StoriesTrainWithDiffusionSameDataset(
            tokenizer=tokenizer,
            file_path=file_path,
            block_size=args.block_size,
            special_words=special_words,
...
def get_data_collator(
        args: DataTrainingArguments,
        tokenizer: PreTrainedTokenizer):
    data_collator = DataCollatorForTimeControlSame(
        tokenizer=tokenizer)
