...
def main():
    ...
    if 'wikihow' in args.model_path:
        args.size = 55
        print ('WARNING: changing size to 55 since wikihow is part of path')
    print (f'NEW SIZE: {args.size}')
    ...
    del training_args['highlevel_train']
    args.__dict__.update(training_args)
    args.sigma_small = True
    #import pdb; pdb.set_trace()

    args.diffusion_steps = 250 #500  # DEBUG
    ...
    print ('---'*5)
    print (f'steps: {diffusion.num_timesteps}')
    print ('---'*5)
    ...
    if 'highlevel' not in args.training_mode:
        model2, tokenizer = load_models(args.modality, args.experiment, args.model_name_or_path, args.in_channel,
                                        os.path.split(args.model_path)[0])
    ...
    dataset = torch.load(args.highlevel_train)
    data = dataset['test']
    data_idx = 0
    args.num_samples = len(data)
    print ('num_smaples', args.num_samples)
    sys.stdout.flush()
    while len(all_images) * args.batch_size < args.num_samples:
        #import pdb; pdb.set_trace()
        gt_zs = data[data_idx:data_idx+args.batch_size]
        data_offset = data_idx
        data_idx += args.batch_size
        gt_zs = torch.stack([item['padded_sentence_embeddings'] for item in gt_zs], dim=0)
        batch_size = min(args.batch_size, gt_zs.shape[0])
        assert args.size == gt_zs.shape[-2]
        size = args.size
        for sss in range(size):
            ...
            #import pdb; pdb.set_trace()
            sample_fn = diffusion.p_sample_infill_loop #(
            ...
            if args.model_arch == '1d-unet':
                ...
                if args.mbr_sample > 1 and args.experiment_mode == 'conditional_gen':
                    ...
                    if 'highlevel' not in args.training_mode:
                        ...
                        sample_shape = (args.batch_size, args.size, args.in_channel)
            print(sample_shape)
            gt_zs = gt_zs.cuda()
            partial_mask = gt_zs.new_zeros(gt_zs.shape).long()
            partial_mask[:, sss, :] = 1
            partial_mask = (partial_mask == 1)
            sample_shape = gt_zs.shape # TODO: addded but not tested yet
            ...
            for bbb in range(batch_size):
                data[data_offset + bbb][f'masked_{sss}'] = sample[bbb][sss]
        ...
        if args.greedy == 1:
            torch.save(data, os.path.join(args.out_dir, os.path.basename(args.highlevel_train)+f'.masked.greedy.seeds.{seed}'))
        else:
            torch.save(data, os.path.join(args.out_dir, os.path.basename(args.highlevel_train)+f'.masked.seeds.{seed}'))
    #torch.save(data, args.highlevel_train+f'.masked.seeds.{seed}')
    if args.greedy == 1:
        torch.save(data, os.path.join(args.out_dir, os.path.basename(args.highlevel_train)+f'.masked.greedy.seeds.{seed}'))
    else:
        torch.save(data, os.path.join(args.out_dir, os.path.basename(args.highlevel_train)+f'.masked.seeds.{seed}'))
    #    torch.save(data, os.path.join(args.output_dir, os.path.basename(args.highlevel_train)+'.masked'))
    #torch.save(data, os.path.join(args.output_dir, os.path.basename(args.highlevel_train)+'.masked'))
    sys.exit(0)
    ...
    if diffusion.training_mode.startswith('highlevel'):
        print('decoding for highlevel', )
        print(arr.shape)
        x_t = th.tensor(arr).cuda()
        torch.save(x_t.cpu(), os.path.join(args.out_dir, 'samples.pt'))
        return 0
