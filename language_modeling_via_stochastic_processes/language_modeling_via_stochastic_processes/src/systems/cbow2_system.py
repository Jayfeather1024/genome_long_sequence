...
class CBow2System(pl.LightningModule):
    ...
    def __init__(self, config):
        ...
        self.use_log = config.model_params.use_log
        print (f'USE LOG: {self.use_log}')
        #import pdb; pdb.set_trace()
...
# now: 0-> t-1, t-> t, T-> t+1
    ...
    def training_step(self, batch, batch_idx):
        ...
        if type(self.use_log) is str:
            sigma = self.log_sigma.exp().sqrt()
        elif self.use_log:
            sigma = self.log_sigma.exp()
        else:
            sigma = self.sigma
        self.log("performance", {'train_ppl': loss.exp().cpu().detach().item(), 'train_acc': acc.cpu().detach().item(), 'sigma': sigma.cpu().detach().item()},  prog_bar=True, on_step=True, batch_size=batch_size)
        return loss
