...
class TM2SepDataset(encoder.BaseDataset):
    ...
    def _set_section_names(self):
        ...
        self.section_ids = ['[ {} ]'.format(name.upper()) for name in self.section_names] + ['[ SEP ]']
    ...
    def _process_data(self):
        ...
        for fname in self.data_files:
            ...
            for conversation in data:
                total_doc_sentences = len(conversation['utterances']) + 1
                #import pdb; pdb.set_trace()
                sentence_counter = 0
                for utterance in conversation['utterances']:
                    current_speaker = utterance['speaker'].upper()
                    text = "[ {} ] {}".format(current_speaker, utterance['text'])
                    ...
                    sentence_counter += 1
### <|endoftext|> [ USER ] x y z. [SEP]|[ AST ]| a b c. [SEP]|[ USER ] d e f. [SEP]|<|endoftext|>
                sentence_info = {
                    "sentence": '<|endoftext|>',
                    "sentence_id": sentence_counter,
                    "doc_id": doc_id,
...
class TM2SepDiscourse(TM2SepDataset):
...
class TM2SepTriplet(TM2SepDataset):
...
class TM2SepCBow(TM2SepDataset):
