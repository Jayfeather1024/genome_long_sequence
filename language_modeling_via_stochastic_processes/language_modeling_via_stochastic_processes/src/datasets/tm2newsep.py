...
class TM2NewSepDataset(encoder.BaseDataset):
    ...
    def _set_tokenizer(self):
        ...
        #self.cl_eos_str = " . "
        if not self.new:
            print ('YES adding tokens')
            self.tokenizer.add_tokens(self.section_ids + [self.cl_eos_str])
        else:
            print ('NOT adding tokens')
            print (self.section_ids)
            self.tokenizer.add_tokens(self.section_ids)
        try:
    ...
    def _process_data(self):
        ...
        for fname in self.data_files:
            ...
            for conversation in data:
                ...
                sentence_info = {
                    "sentence": '<|endoftext|>',
                    "sentence_id": len(conversation['utterances']),
                    "doc_id": doc_id,
                    "total_doc_sentences": len(conversation['utterances'])+1
                }
                self.processed_data.append(sentence_info)
...
class TM2NewSepDiscourse(TM2NewSepDataset):
...
class TM2NewSepTriplet(TM2NewSepDataset):
...
class TM2NewSepCBow(TM2NewSepDataset):
...
class TM2NewSepTPK(TM2NewSepDataset):
