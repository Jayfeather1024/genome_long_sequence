...
def run(config):
    ...
    print("CKPT AT {}".format(
        os.path.join(config.wandb_settings.exp_dir,
        'checkpoints')))
    ckpt_callback = pl.callbacks.ModelCheckpoint(
        os.path.join(config.wandb_settings.exp_dir, 'checkpoints'),
        save_last=True,
        save_top_k=2,
        monitor="val_loss",
        mode='min',
    ...
    system = SystemClass.load_from_checkpoint('/n/holylfs05/LABS/rush_lab/Lab/users/yuntian/language_modeling_via_stochastic_processes/language_modeling_via_stochastic_processes/outputs/2022-06-24/15-00-04/rerun_wikisection_tc32_cbow_finetune_gpt2_single_layer_0.002_32/checkpoints/epoch=96-step=250453.ckpt', config=config)
