...
class GPT2TimeModel(GPT2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if hasattr(config, "dropout_diffusion"):
            self.dropout_diffusion = nn.Dropout(p=config.dropout_diffusion)
        else:
            self.dropout_diffusion = None
    ...
    def _get_cl_embeddings(self, raw_text, cl_feats, seq_cl_feats, input_ids, seq_len):
        ...
        if seq_cl_feats is not None and cl_feats is not None: # Used in evaluation
            if (input_ids.shape[0] > 1 and
                ...
                if ("wikisection" in self.config.dataset_name or
                        ...
                        'stories' in self.config.dataset_name or 
                        'recipe' in self.config.dataset_name): #TODO: add checkpoint and check
        ...
        if generated_by_raw_text:
            ...
            if input_ids.shape[0] == 1:
                cl_embeds = cl_embeds.expand(1, seq_len, self.embed_dim)
            else: # beam
                cl_embeds = cl_embeds.unsqueeze(1)

        return cl_embeds
    def _get_cl_embeddings_batch(self, raw_text, cl_feats, seq_cl_feats, input_ids, seq_len):
        # NOTE assuming batch size 1
        #import pdb; pdb.set_trace()
        generated_by_raw_text = False
        bsz = input_ids.shape[0]
        if type(self._cur_cl_idx) is not list:
            self._cur_cl_idx = [self._cur_cl_idx] * bsz

        if seq_cl_feats is not None and cl_feats is not None: # Used in evaluation
            if False and (input_ids.shape[0] > 1 and
                    (not isinstance(self._transition_cl, list) or
                    not isinstance(self._cur_cl_idx, list))): # beam
                self._transition_cl = [False] * input_ids.shape[0]
                self._cur_cl_idx = [0] * input_ids.shape[0]
                ...
                if ("wikisection" in self.config.dataset_name or
                        ...
                        'recipe' in self.config.dataset_name): #TODO: add checkpoint and check
                    for b in range(bsz):
                        if input_ids[b][0] == self.special_tokens[-1]: # " . " token
                            try:
                                num_feats = seq_cl_feats.shape[0]-1
                            except:
                                num_feats = len(seq_cl_feats)-1
                            self._cur_cl_idx[b] = min(
                                self._cur_cl_idx[b] + 1, num_feats)
                    ...
                    #    self._cur_cl_idx = min(
                    #        self._cur_cl_idx + 1, num_feats)
                    cl_feats = torch.stack([seq_cl_feats[self._cur_cl_idx[b]] for b in range(bsz)], dim=0)
                elif "taskmaster" in self.config.dataset_name:
                    assert False
                    if input_ids[0][0] in self.special_tokens:
                        if not self._has_reset: # don't iterate past
        ...
        if generated_by_raw_text:
            ...
            if True or input_ids.shape[0] == 1:
                cl_embeds = cl_embeds.unsqueeze(1)
                #import pdb; pdb.set_trace()
                pass
                #cl_embeds = cl_embeds.expand(1, seq_len, self.embed_dim)
    ...
    def _get_cl_embeddings_batch_real(self, raw_text, cl_feats, seq_cl_feats, input_ids, seq_len):
        # NOTE assuming batch size 1
        #import pdb; pdb.set_trace()
        generated_by_raw_text = False
        bsz = input_ids.shape[0]
        ...
        if type(self._cur_cl_idx) is not list:
            self._cur_cl_idx = [self._cur_cl_idx] * bsz
        if type(self._has_reset) is not list:
            self._has_reset = [self._has_reset] * bsz
        no_squeeze = False
                ...
                if ("wikisection" in self.config.dataset_name or
                    ...
                    if input_ids.shape[1] > 1:
                        ...
                        for k in range(input_ids.shape[1]):
                            for b in range(bsz):
                                #import pdb; pdb.set_trace()
                                if input_ids[b][k] == self.special_tokens[-1]: # " . " token
                                    try:
                                        num_feats = seq_cl_feats.shape[1]-1
                                    except:
                                        num_feats = len(seq_cl_feats[0])-1
                                    self._cur_cl_idx[b] = min(
                                        self._cur_cl_idx[b] + 1, num_feats)

                            cl_feats.append(torch.stack([seq_cl_feats[b, self._cur_cl_idx[b]] for b in range(bsz)], dim=0).unsqueeze(1))
                        #import pdb; pdb.set_trace()
                        cl_feats = torch.cat(cl_feats, dim=1)
                    else:
                        for b in range(bsz):
                            #import pdb; pdb.set_trace()
                            if input_ids[b][0] == self.special_tokens[-1]: # " . " token
                        ...
                        #    self._cur_cl_idx = min(
                        #        self._cur_cl_idx + 1, num_feats)
                        cl_feats = torch.stack([seq_cl_feats[b, self._cur_cl_idx[b]] for b in range(bsz)], dim=0)
                elif 'tm2new' == self.config.dataset_name or 'tm2newsep' == self.config.dataset_name: # tm2 / 
                    #import pdb; pdb.set_trace()
                    for b in range(bsz):
                        if input_ids[b][0] in [198,]: # " . " token
                            if not self._has_reset[b]:
                                assert False
                                self._has_reset[b] = True
                            else:
                                try:
                                    num_feats = seq_cl_feats.shape[1]-1
                                except:
                    ...
                    #import pdb; pdb.set_trace()
                    for b in range(bsz):
                        if input_ids[b][0] in self.special_tokens[:2]: # " . " token
                            if not self._has_reset[b]:
                                self._has_reset[b] = True
                            else:
                                try:
    ...
    def forward(
        ...
        if self._config.use_contrastive_embeddings:
            ...
            assert seq_cl_feats.shape[0] == input_ids.shape[0]
            #if seq_cl_feats.shape[0] != input_ids.shape[0]:
            #    import pdb; pdb.set_trace()
            #import pdb; pdb.set_trace()
            cl_embeds = self._get_cl_embeddings_batch_real(raw_text=raw_text,
                                                cl_feats=cl_feats,
                                                seq_cl_feats=seq_cl_feats,
                                                input_ids=input_ids,
                                                seq_len=inputs_embeds.shape[1])
            # print(cl_embeds.shape)
            if len(cl_embeds.shape) == 4:
                cl_embeds = cl_embeds.squeeze(1)
            #import pdb; pdb.set_trace()
            if self.dropout_diffusion is None:
                hidden_states = hidden_states + cl_embeds
            else:
                self.dropout_diffusion.train()
                hidden_states = hidden_states + self.dropout_diffusion(cl_embeds)
...
class GPT2TimeLMHeadModel(GPT2PreTrainedModel):
    ...
    def forward(
        ...
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict
        #import pdb; pdb.set_trace()
        if self.not_use_latent == 1:
            seq_cl_feats = seq_cl_feats.fill_(0)
        
        # print("-----------------before_transformer--------------------")
        ...
        #    self.num_words += 1
        #    lm_probs = lm_logits.log_softmax(dim=-1).exp().squeeze()
        #    self.loss += -lm_logits.log_softmax(dim=-1)[..., self.labels[self.step]].item()
        #    lm_logits.fill_(-1000)
        #    lm_logits[..., self.labels[self.step]] = 0
        #    print (math.exp(self.loss/self.num_words))
