...
from transformers import TaskmasterDataset, TaskmasterNewDataset, TaskmasterNewSepDataset
...
class Trainer_Time:
    ...
    def __init__(
        ...
        # Label smoothing
        print ('LABEL SMOOTHING', self.args.label_smoothing_factor)
    ...
    def _get_train_sampler(self) -> Optional[torch.utils.data.sampler.Sampler]:
        ...
        # Build the sampler.
        # print(self.args.group_by_length)
        # print("-----------SAMPLER--------------")
    ...
    def train_dump(
        ...
        **kwargs,
        ...
        #COMMENTED for epoch in range(epochs_trained, num_train_epochs):
        epoch = 0
        if True:
            if isinstance(train_dataloader, DataLoader) and isinstance(train_dataloader.sampler, DistributedSampler):
                train_dataloader.sampler.set_epoch(epoch)
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                #else:
                if True:
                    #tr_loss += self.training_step_dump(model, inputs)
                    model.eval()
                    inputs, sentence_embeddings = self.training_step_dump(model, inputs)
                    inputs = copy.deepcopy(inputs)
                    inputs['sentence_embeddings'] = sentence_embeddings
                    seq_len = sentence_embeddings.shape[0]
                    print (f'Seq len: {seq_len}')
                    for k in inputs:
                        if hasattr(inputs[k], 'detach'):
                            inputs[k] = inputs[k].detach().cpu()
                    all_train_outputs.append(inputs)
            ...
            for step, inputs in enumerate(test_dataloader):
                ...
                for k in inputs:
                    if hasattr(inputs[k], 'detach'):
                        inputs[k] = inputs[k].detach().cpu()
                all_test_outputs.append(inputs)
            data = {'train': all_train_outputs, 'val': all_val_outputs, 'test': all_test_outputs}
            train_max = -1
            #train_max = 57
            print("---------saved_data--------")
            for split in ['train']:
                ...
                for e in ddd:
                    ...
                    train_max = max(train_max, len(sentence_embeddings))
            ...
            for split in data:
                ...
                for e in ddd:
                    ...
                    padded_sentence_embeddings[:e['num_sentences'], :] = sentence_embeddings[:train_max]
            ...
            print (f'DUMPED to {os.path.realpath(fname)}')
            sys.exit(0)
            ...
            #COMMENTED if self.control.should_training_stop:
            #COMMENTED     break
    ...
    def training_step_dump(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        """
        Perform a training step on a batch of inputs.
        ...
        with torch.no_grad():
            ...
            sentence_embeddings = self.compute_loss(model, inputs, return_outputs=True) # seqlen, 32
        return inputs, sentence_embeddings
        ###assert False
    ...
    def compute_loss(self, model, inputs, return_outputs=False, return_outputs2=False):
        """
        ...
        #import pdb; pdb.set_trace()
        if return_outputs:
            sentence_embeddings = inputs['seq_cl_feats'][0]
        ...
        return (loss, outputs) if return_outputs2 else loss
    ...
    def prediction_step(
        ...
        with torch.no_grad():
            if is_sagemaker_mp_enabled():
                ...
                if has_labels:
                    loss, outputs = self.compute_loss(model, inputs, return_outputs2=True)
