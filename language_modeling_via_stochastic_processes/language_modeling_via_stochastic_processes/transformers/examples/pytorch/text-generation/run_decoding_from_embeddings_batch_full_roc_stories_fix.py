...
def main():
    ...
    for sss in range(1, 6):
        with open(os.path.join(args.output_dir, f'oracle{use_oracle}_beam1_max{total_num_samples}_feb14_notuselatent_{model.not_use_latent}_gt_{sss}.txt'), 'w') as fgt:
            with open(os.path.join(args.output_dir, f'oracle{use_oracle}_beam1_max{total_num_samples}_feb14_notuselatent_{model.not_use_latent}_gen_{sss}.txt'), 'w') as fgen:
                for num_example in tqdm.tqdm(range(total_num_samples)):
                    ...
                    if False and (args.use_dataset or args.method == "greedy" or args.method == "beam"):
                        ...
                        row = data[num_example]['raw_text'][0]
                    ...
                    print('[ ACTUAL IN DATA ] {}'.format(row))
...
# AD            DED:###
                    ...
                    for seq_i, (seq_cl_feats, tracker) in enumerate(zip(feats[:1], trackers[:1])):
                        ...
                        # # NOTE Beam search
                            ...
                            num_beams = 5
                            num_beams = 1
                            seq_cl_feats = seq_cl_feats.expand(num_beams, -1, -1)
                            output_sequences = model.generate(
                                input_ids=input_ids,
                                section_ids=None,
                                cl_feats=cl_feats, # .to(args.device),
