...
class GenerationMetrics:
    ...
    def __init__(self, model, device, tokenizer, dataset_name, fname,
        ...
        if len(self.section_names)>0 and ' . ' == self.section_names[-1]:
    ...
    def print_results(self):
        ...
        #fname = os.path.join('results_full', self.fname+"_extreme_examples.csv")
        #fname = os.path.join('results_full', self.fname+"_extreme_examples.csv")
        ...
        #fname = os.path.join('results_full', self.fname+"_metrics.csv")
        fname = self.fname+"_metrics.csv"
        print(f"saving metrics at {fname}")
        with open(fname, "wb") as f:
            df = pd.DataFrame(self._info)
            df.to_csv(f)

        #fname = os.path.join('results_full', self.fname+"_examples.csv")
        fname = self.fname+"_examples.csv"
        with open(fname, "wb") as f:
