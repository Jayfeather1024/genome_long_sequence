...
# ADDED:
        ...
        for seq_i, (seq_cl_feats, tracker) in enumerate(zip(feats[:1], trackers[:1])):
            ...
            if 'tm2fixed' == args.dataset_name:
                model.transformer._has_reset = True
            if 'tm2new' == args.dataset_name:
                model.transformer._has_reset = True
            ...
            for generated_sequence_idx, generated_sequence in enumerate(output_sequences):
                ...
                if debug:
                    ...
                    for d, s, s2 in zip(distances, split_sentences, _):
                        print (f'id {i} ||| sentence: {s.rstrip()} ||||| cosine similarity: {d[0]} (baseline {sum(ddd_sim)/len(ddd_sim)}) |||| l2 distance: {d[1]} (baseline {sum(ddd_dis)/len(ddd_dis)})  ||| actual sentence: {s2.rstrip()}')
                        i += 1
                    import pdb; pdb.set_trace()
                ...
                import pdb; pdb.set_trace()
                if 'tm2sep' == args.dataset_name:
                    text = text.replace('[ SEP ]', '')
                elif 'tm2fixed' == args.dataset_name:
                    #import pdb; pdb.set_trace()
                    import re
