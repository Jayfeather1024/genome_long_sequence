...
def main():
    ...
    gt_cl_tracker = GenerationMetrics(model=model, device=args.device,
                                ...
                                fname=fname+"_trueCLEmbs_" + args.method+args.bridge_version + str(args.num_intervals_factor) + str(args.num_return_sequences),
                                model_args=args,
                                subclass="GT")
    random_cl_tracker = GenerationMetrics(model=model, device=args.device,
                                tokenizer=tokenizer, dataset_name=args.dataset_name,
                            model_args=args,
                                fname=fname+"_randomCLEmbs_"+args.method+args.bridge_version + str(args.num_intervals_factor) + str(args.num_return_sequences),
                                subclass="RANDOM")
    bridge_cl_tracker = GenerationMetrics(model=model, device=args.device,
                                tokenizer=tokenizer, dataset_name=args.dataset_name,
                                fname=fname+"_bridgeCLEmbs_"+args.method+args.bridge_version + str(args.num_intervals_factor) + str(args.num_return_sequences),
                            model_args=args,
                                subclass="BRIDGE")
