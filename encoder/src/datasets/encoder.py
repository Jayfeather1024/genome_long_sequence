...
from transformers import GPT2Tokenizer, BertTokenizer, AlbertTokenizer, PreTrainedTokenizerFast
...
class BaseDataset(data.Dataset):
    ...
    def _set_tokenizer(self):
        if self.tokenizer_name == "GPT2":
            ...
            self.max_length = 512
        elif self.tokenizer_name == "GPT2NeoX":
            ...
            self.tokenizer = PreTrainedTokenizerFast(tokenizer_object=Tokenizer.from_file(tokenizer_path))
    ...
    def tokenize_caption(self, caption, device):
        if self.tokenizer_name == "GPT2":
            caption = [ex for ex in caption if ex is not None]
